...
def load_json(file_path: Path, default: dict, root_dir: Path = None):
    ...
    if root_dir:
        ...
        print(f"[INFO] Paths in {file_path} converted to Path objects using ROOT_DIR={root_dir}")
...
def get_available_cores(default=1):
    try:
        ...
        print(f"[INFO] Allocating {allocated_cores} core(s) to run the models efficiently.")
        return allocated_cores

    except Exception:
        print("[WARN] Unable to detect multiple cores. Defaulting to 1 core for model execution.")
        return default
