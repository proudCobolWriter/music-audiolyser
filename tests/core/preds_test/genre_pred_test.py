...
from music_audiolyser.core.preds.genre_pred import genre_pred
...
def test_genre_pred(monkeypatch):
    monkeypatch.setattr(
        "music_audiolyser.core.preds.genre_pred.TensorflowPredictEffnetDiscogs",
        lambda graphFilename, output: FakeEmbeddingModel(),
    )

    monkeypatch.setattr(
        "music_audiolyser.core.preds.genre_pred.TensorflowPredict2D",
